...
# ============================================================
# Analyze one temperature directory
# ============================================================
...
def analyze_temperature_dir(
    ...
    temp_K = infer_temp_from_path(temp_dir)
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze Tinker-GPU/Tinker-HP NPT outputs in TxxxK directories."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=".",
        help="Root directory containing T250K, T300K, T350K, ...",
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Temperature directories to analyze, e.g. T250K T300K T350K. Default: all T*K directories.",
    )
    ...
    if args.temps is None:
        temp_dirs = sorted(
            [p for p in root.glob("T*K") if p.is_dir()],
            key=lambda p: infer_temp_from_path(p),
        )
    else:
        temp_dirs = [root / t for t in args.temps]
    ...
    if system_mass_g_mol is not None:
        ...
        if args.xyz_for_mass is not None:
            ...
            xyz_for_mass = find_reference_file(root, temp_dirs, ".xyz")

        if args.prm_for_mass is not None:
            prm_for_mass = Path(args.prm_for_mass).resolve()
        else:
            prm_for_mass = find_reference_file(root, temp_dirs, ".prm")
